...
<div class="row g-3 mb-4">
    <div class="col-md-3">
        <div class="card report-card bg-white h-100 position-relative border-bottom border-success border-4">
            <a href="{{ route('clients.index') }}" class="stretched-link no-print"></a>
            <div class="card-body d-flex align-items-center p-3">
                ...
                <div class="ms-3 me-2 flex-grow-1">
                    ...
                    <h4 class="mb-0 fw-bold text-success" id="stat-revenue">{{ number_format($stats['total_revenue']) }}</h4>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card report-card bg-white h-100 position-relative border-bottom border-success border-4">
            ...
            <div class="card-body d-flex align-items-center p-3">
                ...
                <div class="ms-3 me-2 flex-grow-1">
                    ...
                    <h4 class="mb-0 fw-bold text-success" id="stat-cashflow">{{ number_format($stats['expected_cash_flow_total'] ?? 0) }}</h4>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card report-card bg-white h-100 position-relative border-bottom border-primary border-4">
            ...
            <div class="card-body d-flex align-items-center p-3">
                ...
                <div class="ms-3 me-2 flex-grow-1">
                    ...
                    <h4 class="mb-0 fw-bold text-primary" id="stat-clients">{{ $stats['clients_total'] }}</h4>
                    <small class="text-muted fw-bold" id="stat-active-clients"><span class="text-primary">{{ $stats['clients_active'] }} نشط</span></small>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-3">
        <div class="card report-card bg-white h-100 position-relative border-bottom border-secondary border-4">
            <a href="{{ route('users.index') }}" class="stretched-link no-print"></a>
            <div class="card-body d-flex align-items-center p-3">
                ...
                <div class="ms-3 me-2 flex-grow-1">
                    ...
                    <h4 class="mb-0 fw-bold text-secondary" id="stat-users">{{ $stats['users_total'] }}</h4>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-3">
        <div class="card report-card bg-white h-100 position-relative border-bottom border-info border-4"> 
            <a href="{{ route('completed_tasks.index') }}" class="stretched-link no-print"></a> 
            <div class="card-body d-flex align-items-center p-3">
                ...
                <div class="ms-3 me-2 flex-grow-1">
                    ...
                    <h4 class="mb-0 fw-bold text-info" id="stat-completed">{{ $stats['tasks_completed'] }}</h4>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card report-card bg-white h-100 position-relative border-bottom border-secondary border-4">
            <div class="card-body d-flex align-items-center p-3">
                ...
                <div class="ms-3 me-2 flex-grow-1">
                    ...
                    <h4 class="mb-0 fw-bold text-secondary"><span id="stat-avg-res">{{ $stats['avg_resolution_hours'] ?? 0 }}</span> <span class="fs-6">ساعة</span></h4>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-3">
        <div class="card report-card bg-white h-100 position-relative border-bottom border-warning border-4">
            <a href="{{ route('pending_tasks.index') }}" class="stretched-link no-print"></a>
            <div class="card-body d-flex align-items-center p-3">
                ...
                <div class="ms-3 me-2 flex-grow-1">
                    ...
                    <h4 class="mb-0 fw-bold text-warning" id="stat-pending">{{ $stats['tasks_pending'] }}</h4>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-3">
        <div class="card report-card bg-white h-100 position-relative border-bottom border-danger border-4 pulse-danger">
            <a href="{{ route('pending_tasks.index', ['filter' => 'overdue']) }}" class="stretched-link no-print"></a>
            <div class="card-body d-flex align-items-center p-3">
                ...
                <div class="ms-3 me-2 flex-grow-1">
                    ...
                    <h4 class="mb-0 fw-bold text-danger" id="stat-overdue">{{ $stats['tasks_overdue'] ?? 0 }}</h4>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card border-0 shadow-sm rounded-4 h-100">
            ...
            <div class="card-body d-flex flex-column justify-content-center gap-3">
                <div class="aging-box bg-success bg-opacity-10 border-success border-opacity-25">
                    ...
                    <h3 class="fw-bold text-success mb-0" id="aging-new">{{ $aging['new'] ?? 0 }}</h3>
                </div>
                <div class="aging-box bg-warning bg-opacity-10 border-warning border-opacity-50">
                    ...
                    <h3 class="fw-bold text-dark mb-0" id="aging-warning">{{ $aging['warning'] ?? 0 }}</h3>
                </div>
                <div class="aging-box bg-danger bg-opacity-10 border-danger border-opacity-25">
                    ...
                    <h3 class="fw-bold text-danger mb-0" id="aging-danger">{{ $aging['danger'] ?? 0 }}</h3>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-6" id="renewals-section">
        <div class="card border-0 shadow-sm rounded-4 h-100">
            <div class="card-header bg-white border-bottom pt-3 pb-2 d-flex justify-content-between">
                ...
                <span class="badge bg-success bg-opacity-10 text-success border border-success px-3 fs-6" id="cash-flow-total">{{ number_format($stats['expected_cash_flow_total']) }} ريال</span>
            </div>
            <div class="card-body p-0" id="cash-flow-list" style="max-height: 280px; overflow-y: auto;">
                @forelse($cashFlowClients as $cfc)
                    <div class="d-flex justify-content-between align-items-center p-3 border-bottom">
                        <div>
                            <h6 class="mb-1 fw-bold text-dark">
                                <a href="{{ route('clients.show', $cfc->id) }}" class="text-decoration-none text-dark hover-zoom d-inline-block">{{ $cfc->name }} <i class="fas fa-external-link-alt small ms-1 opacity-50"></i></a>
                            </h6>
                            <small class="text-muted"><i class="far fa-calendar-alt text-danger"></i> ينتهي في: {{ \Carbon\Carbon::parse($cfc->sub_end_date)->format('Y-m-d') }}</small>
                        </div>
                        <div class="fw-bold text-success">{{ number_format($cfc->subscription_amount) }} ريال</div>
                    </div>
                @empty
        ...
        <div class="card border-0 shadow-sm rounded-4 h-100">
            ...
            <div class="card-body p-0" id="top-clients-list" style="max-height: 280px; overflow-y: auto;">
                @forelse($topClients ?? [] as $c)
                    @php /** @var \App\Models\Client $c */ @endphp
                    <div class="d-flex justify-content-between align-items-center p-3 border-bottom hover-zoom">
                        <div>
                            <h6 class="mb-1 fw-bold text-dark"><a href="{{ route('clients.show', $c->id) }}" class="text-decoration-none text-dark"><i class="fas fa-building text-primary me-2 opacity-50"></i>{{ $c->name }}</a></h6>
                            <small class="text-muted fw-bold">العائد: <span class="text-success">{{ $c->subscription_amount ? number_format($c->subscription_amount) . ' ريال' : 'غير محدد' }}</span></small>
                        </div>
        ...
        <div class="card border-0 shadow-sm rounded-4 h-100">
            ...
            <div class="card-body" style="max-height: 600px; overflow-y: auto;">
                @forelse($recentOperations as $op)
                    ...
                    @endphp
                    <div class="timeline-item mb-3" style="border-right-color: {{ $color }};">
                        <div class="d-flex justify-content-between mb-1">
                            <span class="fw-bold text-dark small"><i class="fas {{ $icon }} me-1" style="color: {{ $color }}"></i> {{ $op->user->username ?? 'نظام' }}</span>
                            <span class="text-muted" style="font-size: 0.70rem;" dir="ltr">{{ \Carbon\Carbon::parse($op->created_at)->format('m-d h:i A') }}</span>
                        </div>
                        <div class="mb-1 text-secondary" style="font-size: 0.8rem; line-height: 1.5;">
                            {!! $op->formatted_text ?? strip_tags($op->action_text, '<b><i><u><strong><em><br><a><span>') !!}
                        </div>
                        @if($op->client)
                            <a href="{{ route('clients.show', $op->client->id) }}" class="badge bg-light text-dark text-decoration-none border shadow-sm no-print mt-1 hover-zoom"><i class="fas fa-building text-muted"></i> {{ $op->client->name }}</a>
                        @endif
                    </div>
                @empty
...
<script>
    ...
    var productivityChart = new Chart(ctxProd, {
        type: 'line',
        data: {
            labels: @json($prod_labels ?? []),
            datasets: [{
                label: 'مهام منجزة',
                data: @json($prod_counts ?? []),
    ...
    var empPerformanceChart = new Chart(ctxEmp, {
        type: 'bar',
        data: {
            labels: @json($emp_labels ?? []),
            datasets: [{
                label: 'مهام منجزة',
                data: @json($emp_counts ?? []),
    ...
    function updateDashboard(period) {
        ...
        fetch(`{{ route('reports.index') }}?period=${period}`, { headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' } })
    ...
    document.addEventListener('DOMContentLoaded', function() {
        var calendar = new FullCalendar.Calendar(document.getElementById('calendar'), {
            ...
            events: @json($events ?? []), eventDisplay: 'block', contentHeight: 'auto', dayMaxEvents: 3,
