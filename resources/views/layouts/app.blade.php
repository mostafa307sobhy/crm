...
<head>
    ...
    <title>بيت المحاسبة - @yield('title', 'لوحة التحكم')</title>
    ...
    <div class="wrapper">
        <nav id="sidebar">
            ...
            <ul class="list-unstyled sidebar-menu">
                ...
                <li>
                    <a href="{{ route('clients.index') }}" class="nav-link {{ request()->routeIs('clients.*') ? 'active-link' : '' }}">
                ...
                <li>
                    <a href="{{ route('system_logs.index') }}" class="nav-link {{ request()->routeIs('system_logs.*') ? 'active-link' : '' }}">
        ...
        <div id="content" class="bg-light">
            ...
            <nav class="navbar top-navbar d-flex justify-content-between align-items-center">
                <div class="d-flex align-items-center gap-3">
                    ...
                    <h5 class="mb-0 fw-bold text-dark d-none d-md-block">@yield('title', 'مرحباً بك')</h5>
                </div>
                ...
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div class="dropdown">
                        <button class="btn btn-light border-0 bg-transparent d-flex align-items-center gap-2 dropdown-toggle shadow-none" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                            ...
                            <span class="fw-bold text-dark">{{ auth()->user()->username ?? 'مستخدم' }}</span>
                        </button>
                        <ul class="dropdown-menu shadow-lg border-0 rounded-4 p-2" style="min-width: 200px; left: 0 !important; right: auto !important; position: absolute;">
                            <li>
                                <a class="dropdown-item py-2 fw-bold text-secondary rounded" href="{{ route('profile.index') }}">
                            ...
                            <div class="border rounded-3 p-3 bg-white shadow-sm employees-box" style="max-height: 150px; overflow-y: auto; display: none;">
                                @foreach(\App\Models\User::where('role', '!=', 'admin')->get() as $u)
                                    <div class="form-check mb-2 border-bottom pb-1">
                                        <input class="form-check-input" type="checkbox" name="assigned_users[]" value="{{ $u->id }}" id="add_u_{{ $u->id }}">
                                        <label class="form-check-label small text-dark fw-bold" for="add_u_{{ $u->id }}">{{ $u->username }}</label>
                                    </div>
                                @endforeach
                            </div>
                        </div>
...
<script>
    ...
    function handleNotifClick(id, url) {
        fetch(`/notifications/read/${id}`, { method: 'POST', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' } })
